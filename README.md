...
- `workers/`: Celery worker logic for async dispatch execution
...
- UI -> API -> Redis queue -> Celery workers -> external target service
- Optional webhook callbacks after job completion
- Flower dashboard for worker/queue visibility
...
- If dispatches remain queued: check Redis container health and worker logs.
